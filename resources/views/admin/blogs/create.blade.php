...
<div class="max-w-4xl mx-auto">
    ...
    <div class="flex items-center gap-4 mb-8">
        ...
        <div>
            <h1 class="text-3xl font-bold text-slate-900 tracking-tight">Create Blog Post</h1>
            <p class="text-slate-500 mt-1">Publish a new article, news post, or case study.</p>
        </div>
    </div>
    ...
    <div class="bg-white rounded-2xl shadow-sm border border-slate-100 p-6 sm:p-8" x-data="{ 
        ...
        <form action="{{ route('admin.blogs.store') }}" method="POST" enctype="multipart/form-data" class="space-y-6" hx-boost="false">
            @csrf
            ...
            <div>
                ...
                <textarea name="summary" rows="3" placeholder="Provide a brief summary for the blog card preview..." class="w-full px-4 py-3 rounded-xl border border-slate-200 bg-slate-50 focus:bg-white focus:ring-2 focus:ring-indigo-500 outline-none transition-all text-sm font-medium text-slate-700"></textarea>
            ...
            <div class="flex justify-end gap-4 border-t border-slate-100 pt-6">
                ...
                <button type="submit" class="px-6 py-2.5 bg-indigo-600 hover:bg-indigo-700 text-white rounded-xl text-sm font-bold shadow-sm transition-colors">Publish Post</button>
            </div>
        </form>
    </div>
</div>
